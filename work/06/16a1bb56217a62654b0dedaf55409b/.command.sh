...
# Run BLAST with 12-field outfmt (matching reference qc_oriv_arg2.py)
    echo -e "qseqid\tsseqid\tpident\tlength\tevalue\tbitscore\tqstart\tqend\tqlen\tsstart\tsend\tslen" > McClain_PlasmidGPT_RL_GFP_cassette.ori_hits.tsv
    ...
    blastn \
        ...
        -task dc-megablast \
        -soft_masking true \
        -dust yes >> McClain_PlasmidGPT_RL_GFP_cassette.ori_hits.tsv
...
df = pd.read_csv("McClain_PlasmidGPT_RL_GFP_cassette.ori_hits.tsv", sep='\t')
...
df.to_csv("McClain_PlasmidGPT_RL_GFP_cassette.ori_calls.csv", index=False)
PYEOF

    # Replace raw TSV with filtered CSV for aggregation
    mv McClain_PlasmidGPT_RL_GFP_cassette.ori_calls.csv McClain_PlasmidGPT_RL_GFP_cassette.ori_hits.tsv
