...
# Run BLAST with 12-field outfmt (matching reference qc_oriv_arg2.py)
    echo -e "qseqid\tsseqid\tpident\tlength\tevalue\tbitscore\tqstart\tqend\tqlen\tsstart\tsend\tslen" > UCL_CSSB_PlasmidGPT_ATG.ori_hits.tsv
    ...
    blastn \
        ...
        -task dc-megablast \
        -soft_masking true \
        -dust yes >> UCL_CSSB_PlasmidGPT_ATG.ori_hits.tsv
...
df = pd.read_csv("UCL_CSSB_PlasmidGPT_ATG.ori_hits.tsv", sep='\t')
...
df.to_csv("UCL_CSSB_PlasmidGPT_ATG.ori_calls.csv", index=False)
PYEOF

    # Replace raw TSV with filtered CSV for aggregation
    mv UCL_CSSB_PlasmidGPT_ATG.ori_calls.csv UCL_CSSB_PlasmidGPT_ATG.ori_hits.tsv
